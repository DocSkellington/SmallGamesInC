...
static void init(void **memory, StateManager *manager) {
  ...
  m->gameOver = SDL_CreateTextureFromSurface(renderer, surface);
...
static void render(void *memory, SDL_Renderer *renderer) {
  ...
  SDL_GetTextureSize(m->gameOver, &gameOverDst.w, &gameOverDst.h);
  ...
  SDL_RenderTexture(renderer, m->gameOver, nullptr, &gameOverDst);
